...
public class Battleship {
  ...
  public static void main(String[] args) {
    ComputerBrain b = new ComputerBrain();
    ...
    while (true) {
      ...
      compMakeGuess(computer, userPlayer, b);
    }
  }

  private static void compMakeGuess(Player comp, Player user, ComputerBrain brain) {
    int[] guess = brain.makeGuess(user, comp);
    System.out.println(Arrays.deepToString(brain.generateProbabilityMap()));
    int col = guess[0];
    int row = guess[1];
    ...
    while (p.numOfShipsLeft() > 0) {
      for (Ship s : p.ships) {
        ...
        while (true) {
          ...
          {
            if (!hasErrors(row, col, dir, p, normCounter)) // Check if errors will produce (out of
                                                           // bounds)
            {
              break;
            }
  ...
  private static void setupComputer(Player p) {
    ...
    while (p.numOfShipsLeft() > 0) {
      for (Ship s : p.ships) {
        ...
        while (hasErrorsComp(row, col, dir, p, normCounter)) // while the random nums make error,
                                                             // start again
        {
